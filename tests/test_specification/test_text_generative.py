...
class TestGenerativeModelSpecification(unittest.TestCase):
    ...
    def test_searching_spec(self):
        ...
        for i, dataset_name in enumerate(["pubmedqa", "medmcqa"]):
            ...
            specs.append(spec)
            learnwares.append(Learnware(str(i), None, Specification(stat_spec={spec.type: spec}), ""))

        searcher = LLMStatSearcher(None)
        searcher._search_by_taskvector_spec_single(learnwares, specs[-1], specs[-1].type)
