...
def prepare_learnware(data_path, model_path, init_file_path, yaml_path, save_root, zip_name):
    ...
    user_spec = specification.TextRKMEStatSpecification()
    user_spec.generate_stat_spec_from_text(X=X)
    ed = time.time()
...
def test_search(gamma=0.1, load_market=True):
    ...
    logger.info(
        "Ensemble Reuse Performance: %.3f +/- %.3f" % (np.mean(pruning_score_list), np.std(pruning_score_list))
    )
