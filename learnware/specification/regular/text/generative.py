...
class GenerativeModelSpecification(TaskVectorSpecification):
    ...
    def generate_stat_spec_from_data(
        ...
        **kwargs,
        ...
        self._task_vector = torch.concatenate([(p1 - p0).reshape(-1) for p0, p1 in zip(param_0, param_1)])
    ...
    def save(self, filepath: str):
        torch.save({"type": self.type, "task_vector": self.task_vector.detach().cpu()}, filepath)

    def load(self, filepath: str):
        state = torch.load(filepath, weights_only=True)
        if state["type"] != self.type:
            logger.warning("{} may not be consistent with this class {}.".format(state["type"], self.type))
...
class CustomSFTTrainer(trl.SFTTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        default_loss, outputs = super().compute_loss(
